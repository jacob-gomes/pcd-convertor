...
public class NumberToWords {
    ...
    static public class DefaultProcessor extends AbstractProcessor {
            ...
            public String getName(String value) {
                ...
                boolean zeroDecimal = false;
                if(null == decimalValue || "".equals(decimalValue.trim())) {
                    zeroDecimal = true;
                }
                else {
                    if( 0 == Integer.valueOf(decimalValue).intValue()) {
                        zeroDecimal = true;
                    }
                }

                if (!zeroDecimal) {
                    name = name.concat(" ").concat(UNION_AND).concat(" " + (Double.valueOf(decimalValue)).intValue()).concat("/100 Only");
                }
                else {
                    name = name.concat(" ").concat(UNION_AND).concat(" None/100 Only");
                }
...
*/
    ...
    public static String convertAmountToText(String str) {
        processor = new DefaultProcessor();
        return processor.getName(str);
    }
}
